class AutoLikeWorker
  ...
  def perform(id, prefix)
    init id, prefix
    last_time = Time.parse($redis.HGET(prefix, 'last_time') || @like.updated_at.to_s) # 最后点赞时间
    last_update = @like.updated_at # 最后保存时间
    ...
    AutoLikeWorker.perform_at(5.hours.from_now, id, prefix) if @like # 五小时后再判断自动保存
  rescue => e
    # 存在一种情况，在异步保存点赞数据时原文章等已被作者删除
    puts e
    $redis.DEL prefix unless @like
  ...
  def save
    t = Time.now
    if $redis.HLEN(@prefix) == 1 # 没有人点赞时(唯一key为:last_time)删除like
      @like.resource.update tbp_counts: 0
      $redis.DEL @prefix
      @like.delete
      @like = nil
    else
      counts = $redis.hlen(@prefix) - 1 # 获取点赞数，减去last_time
      @like.update! likes: to_hash($redis.HGETALL(@prefix)), likes_count: counts
      @like.resource.update tbp_counts: counts
