class Index::Like < ApplicationRecord
  ...
  #----------------------------域------------------------------
  scope :t_counts, ->{ select(:likes_count) }


  #-------------------------- 添加赞 ------------------------#
  def self.add(resource, user)
    prefix(resource)
    init_like(resource) if @tmp_prefix
  ...
  def self.has?(resource, user)
    prefix(resource)
    if @tmp_prefix
      set_likes(resource).include? user.id.to_s
    else
  ...
  def self.counts_and_has?(resource, user)
      prefix(resource)
      if @tmp_prefix
        likes = set_likes(resource)
        counts = likes.size
        has = user ? likes.include?(user.id.to_s) : false
      else
  ...
  def self.prefix resource
    @prefix = "index_#{resource.id}_#{resource.file_type}_likes"
    @tmp_prefix = nil
    $redis.EXISTS(@prefix)
  ...
  def self.init_like resource
    if $redis.EXISTS(@tmp_prefix) == 0 # 判断是否已经进行异步加载
      LikeWorker.perform_async(resource.file_type, resource.id, @prefix, @tmp_prefix)
    end
    @prefix = @tmp_prefix
  end

  def self.set_likes resource
    counts = resource.tbp_counts || 0
    if counts > 0 && counts < 10000
      resource.add_like likes.merge(to_hash($redis.HGETALL(@tmp_prefix)))
    else
