class Index::Workspace::Trash < ActiveRecord::Base
    ...
    begin
      ActiveRecord::Base.transaction do
        if file.is_root? # 根文件
          ...
          file_seed.update! is_deleted: true
        else
          files = file.files
  ...
  #--------------------------恢复文件--------------------------- #
  def recover_files
    return false unless file.is_deleted
    file = self.file
      ...
      ActiveRecord::Base.transaction do
        if file.is_root? # 根文件
          ...
          when :articles
            # do nothing
          when :corpuses
            file.son_articles_with_deleted_with_content.update_all is_deleted: false
          else # :folders
            files = file_seed.files
          ...
          file_seed.update! is_deleted: false
        else
          files = file.files
      ...
      ActiveRecord::Base.transaction do # 出错将回滚
        if file.is_root? # 根文件
          ...
          unless file.file_type == :articles # 文章可以直接删除, 无需以下预加载
            file_seed = Index::Workspace::FileSeed.deleted.where(id: file_seed.id).includes(:trashes, :editor_roles, :folders_with_deleted, corpuses_with_deleted: [:comments, :thumb_up], articles_with_deleted: [:comments, :edit_comments, :thumb_up]).first
          end
